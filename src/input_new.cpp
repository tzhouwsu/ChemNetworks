...
void ChemNetworkNew::input_read(int argc, char *argv[]){
  ...
  for(i=0; i < NUM_INTER; i++) {    // NUM_INTER is defined in common.h, and the array is defined in chemnetworks_new.h, Tiecheng
    ...
    // default upper cutoff in angle of solvent-solvent, (real angle is in the range of 0.0 to 180.0)
    s1s1v7[i] = s2s2v7[i] = s3s3v7[i] = s1s2v7[i] = s1s3v7[i] = s2s3v7[i] = 181.0;
    
    // defalut lower value in distance of solvent-solvent, (real distance is in the range of 0.0 to 100.0)
    s1as1bBDmin[i] = s2as2bBDmin[i] = s3as3bBDmin[i] = s12as12bBDmin[i] = s13as13bBDmin[i] = s23as23bBDmin[i] = -0.1;
    
    // defalut upper value in distance of solvent-solvent, (real distance is in the range of 0.0 to 100.0)
    s1as1bBDmax[i] = s2as2bBDmax[i] = s3as3bBDmax[i] = s12as12bBDmax[i] = s13as13bBDmax[i] = s23as23bBDmax[i] = 100.0;

    // defalut lower value in distance of solvent-solute, (real distance is in the range of 0.0 to 100.0)
    s1t1cutoffmin[i] = s1t2cutoffmin[i] = s2t1cutoffmin[i] = s2t2cutoffmin[i] = s3t1cutoffmin[i] = s3t2cutoffmin[i] = -1.0;

    // defalut upper value in distance of solvent-solute, (real distance is in the range of 0.0 to 100.0)
    s1t1cutoffmax[i] = s1t2cutoffmax[i] = s2t1cutoffmax[i] = s2t2cutoffmax[i] = s3t1cutoffmax[i] = s3t2cutoffmax[i] = 100.0;
  }
  ...
  // Start reading Input Files
  {
    ...
    if(num_xyz_files != (nsolventtype + nsolutetype)){
      printf("Usage: %s inputfile coordinate-file(s)\n"
	     "inputfile is assumed to have extension .input\n"
	     "coordinate-file(s) is(are) assumed to have extension .xyz\n", argv[0]);
      
      fprintf(stdout,"num_xyz_files= %i  expecting= %i\n",num_xyz_files,nsolventtype+nsolutetype);
      fprintf(stdout,"nsolventtype= %i  nsolutetype= %i\n",nsolventtype,nsolutetype);
      for(int i=0; i<num_command_arguments; ++i) fprintf(stdout,"argv= %s\n",argv[i]);
...
void ChemNetworkNew::read_command_line(int argc, char *argv[])
{
  num_command_arguments = argc;

  // defaults
  
  using_new_interface = 0;
  num_xyz_files = 0;
  strcpy(filename_input, "cn.input");
  strcpy(filename_xyz,   "cn.xyz");

  // initialize global variables

  config_indx = -1;

  // check if 'new' interface being used
  //  new interface only supports solvent1 currently

  for(int i=0; i<argc; ++i) if(strcmp(argv[i],"-new") == 0) using_new_interface = 1;

  if(using_new_interface) {

    num_xyz_files = 1;
    strcpy(filename_input, argv[1]);
    strcpy(filename_xyz,   argv[2]);
    
  } else {
    num_xyz_files = argc - 2;
    
    if(num_xyz_files < 1){
      fprintf(stdout,"Usage: %s inputfile coordinate-file(s)\n"
	      "inputfile is assumed to have extension .input\n"
	      "coordinate-file(s) is(are) assumed to have extension .xyz\n", argv[0]);

      fprintf(stdout,"num_command_arguments= %i\n",num_command_arguments);
      fprintf(stdout,"num_xyz_files= %i\n",num_xyz_files);
      for(int i=0; i<num_command_arguments; ++i) fprintf(stdout,"argv= %s\n",argv[i]);
...
void ChemNetworkNew::read_xyz_solvent1(char filename[])
{
  config_indx++;
  
  // the first xyz file is the solvent1
  
  if((fsolvent1 = fopen(filename,"r")) == NULL){
    printf("read_xyz_solvent1::Cannot open filename= %s\n", filename);
    exit(-1);
  }
  ...
  for(i=0; i < nAtomS1; i++){
    if(fscanf(fsolvent1, "%31s %lf %lf %lf", atmTypeS1[i], &atmS1[3*i], &atmS1[3*i+1], &atmS1[3*i+2]) != 4){
  ...
  for(i=0; i<nsolvent1; ++i) {
    if(strcmp(slvntatm1[i], atmTypeS1[i]) != 0) {
      printf("Solvent1 XYZ file: Atom types and/or their order do not match with the input file\n");
      exit(-1);
    }
  }
...
void ChemNetworkNew::load_xyz_solvent1(int n, char ** type, double * xyz)
{
  config_indx++;
  ...
  if(config_indx == 0) {
    atmS1 = (double*)malloc((3*n)*sizeof(double));
  
    atmTypeS1 = (char **)malloc(sizeof(char *)*n);
    for(i=0; i < n; i++) atmTypeS1[i] = (char *)malloc(sizeof(char)*32);
  }

  nAtomS1 = n;
  for(int i=0; i<n; ++i) strcpy(atmTypeS1[i], type[i]);
  for(int i=0; i<3*n; ++i) atmS1[i] = xyz[i];
}

/* -------------------------------------------------------------------------- */

void ChemNetworkNew::read_xyz_all(char *argv[])
{
  // Only solvent1 currently supported in new interface
  
  if(using_new_interface) {
    read_xyz_solvent1(filename_xyz);
...
void ChemNetworkNew::process_config(char *argv[])
{  
  // General Output to LOG File
  
  if((nsolventtype + nsolutetype) == 1) sprintf(foutput, "%s.%s.log",             argv[1], argv[2]);
  if((nsolventtype + nsolutetype) == 2) sprintf(foutput, "%s.%s.%s.log",          argv[1], argv[2], argv[3]);
  if((nsolventtype + nsolutetype) == 3) sprintf(foutput, "%s.%s.%s.%s.log",       argv[1], argv[2], argv[3], argv[4]);
  if((nsolventtype + nsolutetype) == 4) sprintf(foutput, "%s.%s.%s.%s.%s.log",    argv[1], argv[2], argv[3], argv[4], argv[5]);
  if((nsolventtype + nsolutetype) == 5) sprintf(foutput, "%s.%s.%s.%s.%s.%s.log", argv[1], argv[2], argv[3], argv[4], argv[5], argv[6]);
  ...
  */
  ...
  if(gs1s1 == 1) {   // 2015.12.14, Output file name format here, Tiecheng
    if(using_new_interface) {
      sprintf(foutputGraphS1S1, "%s.%s.%s.%i.Graph", filename_input, filename_xyz, filename_xyz, config_indx);
      sprintf(foutputGeodS1S1, "%s.%s.%s.%i.GraphGeod", filename_input, filename_xyz, filename_xyz,config_indx);
      if(pnumnodes==1) sprintf(foutputNumnodesS1S1, "%s.%s.%s.%i.GraphNumnodes", filename_input, filename_xyz, filename_xyz, config_indx);
    } else {
      sprintf(foutputGraphS1S1, "%s.%s.%s.Graph", filename_input, filename_xyz, filename_xyz);
      sprintf(foutputGeodS1S1, "%s.%s.%s.GraphGeod", filename_input, filename_xyz, filename_xyz);
      if(pnumnodes==1) sprintf(foutputNumnodesS1S1, "%s.%s.%s.GraphNumnodes", filename_input, filename_xyz, filename_xyz);
    }
    
    outputfGraphS1S1 = fopen(foutputGraphS1S1,"w");
    outputfGeodS1S1  = fopen(foutputGeodS1S1,"w");
    if(pnumnodes==1) outputfNumnodesS1S1 = fopen(foutputNumnodesS1S1,"w");
    
    // Construct the solvent1-solvent1 graph
   
    // add energetic calculation for water (as solvent1-solvent1)
    if(E_s1s1_num > 0 && E_s1s1_charge_num > 0 && E_s1s1_LJ_num > 0)
    {
       ...
       else  // if solvent1 is not water, use the previous 'graph_ss'
       {
          printf("Warning: energetic calculation is only implimented for water\n");

          graph_ss(atmS1, 1, nsolvent1, nAtomS1, s1s1hbdn, s1a, s1b, s1as1bBDmin, s1as1bBDmax, s1s1hban,
                   s1s1v1, s1s1v2, s1s1v3, s1s1v4, s1s1v5, s1s1v6, s1s1v7,
                   pbc, xside, yside, zside, outputfGraphS1S1, outputfGeodS1S1);    // 2015.12.15, I changed here, Tiecheng

       }
    }
    else   // when the pair energy is not requested, use the previous 'graph_ss'
    {
       graph_ss(atmS1, 1, nsolvent1, nAtomS1, s1s1hbdn, s1a, s1b, s1as1bBDmin, s1as1bBDmax, s1s1hban,
                s1s1v1, s1s1v2, s1s1v3, s1s1v4, s1s1v5, s1s1v6, s1s1v7,
                pbc, xside, yside, zside, outputfGraphS1S1, outputfGeodS1S1);    // 2015.12.15, I changed here, Tiecheng
  ...
  if(findpdfshell==1) {
    ...
    for(i=0; i<maxshellsize; i++) shelldistmtx[i]=(double*)malloc(maxshellsize*sizeof(double));      
  ...
  if(!using_new_interface) deallocate_solvent(nsolvent1, slvntatm1, nAtomS1, atmTypeS1, atmS1);

  if(nsolventtype == 2 || nsolventtype == 3) deallocate_solvent(nsolvent2, slvntatm2, nAtomS2, atmTypeS2, atmS2);

  if(nsolventtype == 3) deallocate_solvent(nsolvent3, slvntatm3, nAtomS3, atmTypeS3, atmS3);

  if(nsolutetype != 0) deallocate_solvent(nsolute1, sltatm1, nAtomT1, atmTypeT1, atmT1);

  if(nsolutetype == 2) deallocate_solvent(nsolute2, sltatm2, nAtomT2, atmTypeT2, atmT2);
    
  if(findpdfshell==1) {
    for(i=0; i<maxshellsize; i++) free(shelldistmtx[i]);
    free(shelldistmtx);
  }
  ...
  if(gdist == 1 && gdistS1S2 == 1) {
    ...
    geodesics_sAsB(nsolvent1, nAtomS1, nsolvent2, nAtomS2, EucDistS1S2, EucRefS1S2s1, EucRefS1S2s2,
		   xside, yside, zside, foutputGeodS1S2, finput, finput2);
  }
  ...
  if(gdist == 1 && gdistS1S3 == 1) {
    ...
    geodesics_sAsB(nsolvent1, nAtomS1, nsolvent3, nAtomS3, EucDistS1S3, EucRefS1S3s1, EucRefS1S3s3,
		   xside, yside, zside, foutputGeodS1S3, finput, finput3);
  }
  ...
  if(gdist == 1 && gdistS2S3 == 1) {
    ...
    geodesics_sAsB(nsolvent2, nAtomS2, nsolvent3, nAtomS3, EucDistS2S3, EucRefS2S3s2, EucRefS2S3s3,
		   xside, yside, zside, foutputGeodS2S3, finput2, finput3);
  }
  ...
  if(gdist == 1 && gdistS1S2S3 == 1) {
    ...
    geodesics_sAsBsC(nsolvent1, nAtomS1, nsolvent2, nAtomS2, nsolvent3, nAtomS3,
		     EucDistS1S2S3, EucRefS1S2S3s1, EucRefS1S2S3s2, EucRefS1S2S3s3,
		     xside, yside, zside, foutputGeodS1S2S3, finput, finput2, finput3);
  }
