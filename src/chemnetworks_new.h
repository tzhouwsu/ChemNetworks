...
#ifndef CHEMNETWORKS_NEW_H
#define CHEMNETWORKS_NEW_H
...
namespace CN_NS {
  ...
  class ChemNetworkNew : public ChemNetwork {
  public : 
    ChemNetworkNew();
    ~ChemNetworkNew() {};
    ...
    void read_xyz_all(char *[]);
    void read_xyz_solvent1(char[]);
    void load_xyz_solvent1(int, char**, double*);
    
    void process_config(char * []);
  ...
  private : 
    ...
    int num_command_arguments;
    
    int using_new_interface;
    int num_xyz_files;
    char filename_input[MAX_CHAR_LENGTH];
    char filename_xyz[MAX_CHAR_LENGTH];

    int config_indx;

    void deallocate_solvent(int, char **, int, char **, double *);
