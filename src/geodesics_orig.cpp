...
void ChemNetworkOrig::geodesics_ss(int nsolvent, int nAtomS, int EucDistS, int EucRefS, double xside, double yside, double zside, char *foutputGeodSS, char *finput)
{
...
if(EucDistS == 1)
{
  ...
  if((fxyzS = fopen(finput,"r")) == NULL){
   ...
   outputfCard = fopen(foutputCard, "w");
        ...
        for (i=0; i < numnodes; i++)
        {
//            free(coord[i]);    // coord[] is only allocate when EucDistS=1
            free(gdmatrix[i]);
            free(adjmatrix[i]);
            free(next[i]);
        }
...
void ChemNetworkOrig::geodesics_sAsB(int nsolventA, int nAtomSA, int nsolventB, int nAtomSB, int EucDistSASB, int EucRefSASBsa, int EucRefSASBsb, double xside, double yside, double zside, 
...
void ChemNetworkOrig::geodesics_sAsBsC(int nsolventA, int nAtomSA, int nsolventB, int nAtomSB, int nsolventC, int nAtomSC, int EucDistSASBSC, int EucRefSASBSCsa, int EucRefSASBSCsb, int EucRefSASBSCsc, 
...
if(EucDistSASBSC == 1)
{
  ...
  if((fxyzSC = fopen(finputC,"r")) == NULL){
   ...
   outputfCard = fopen(foutputCard, "w");
        ...
        for (i=0; i < numnodes; i++)
        {
//            free(coord[i]);    // coord[] is only allocate when EucDistSASBSC=1
            free(gdmatrix[i]);
            free(adjmatrix[i]);
            free(next[i]);
        }
...
void ChemNetworkOrig::GetPath(int i, int j, int **next, int **gdmatrix, FILE *fout)
{        
    if (gdmatrix[i][j] == 1)
    {
        fprintf(fout, "%d ", i+1);
...
int ChemNetworkOrig::FindChange(int startnode, int endnode, struct edge* edgelist, int numedgelist)
{
